...
def test_g4_readiness_audit_accepts_frozen_preregistration_with_qpu_locked() -> None:
    report = build_g4_readiness_audit(Path("."))
    checks = {check["id"]: check for check in report["checks"]}

    assert report["overall_status"] == "ready_for_h501"
    assert checks["G3-Q302"]["status"] == "pass"
    assert checks["H401-PREREG-FIELDS"]["status"] == "pass"
    assert checks["H401-HUMAN-APPROVAL"]["status"] == "pass"
    assert checks["H403-QPU-GATE"]["status"] == "pass"
    assert checks["H403-DRY-RUN"]["status"] == "pass"
