...
it submits no workload. The current provider snapshot selects
\texttt{ibm\_kingston} with physical layout \([125,117,126,124]\), native
\texttt{cz} entangling gates and calibration timestamp
\texttt{2026-06-20T20:55:08+03:00}.
...
Q1 and Q2 remain exact/simulator illustrations only. Q304 is now a
backend-derived local readiness simulation, not a QPU measurement. This
preserves the paper boundary that the classical/symbolic contribution survives
without IBM hardware.
...
file names without writing secret values to any artifact. It found one Qiskit
account-file token source and records only key paths plus non-secret runtime
metadata.

The current status remains \texttt{blocked\_before\_g4}. Q302 generic local
readiness, Q303 provider selection, the Open Plan instance check, Q304
backend-derived local twin, the QPU-disabled gate and the pushed repository
snapshot pass. Two checks still block G4: the frozen preregistration fields are
not complete, and explicit human approval is not recorded. The H401 draft packet
narrows the concrete unresolved fields to the environment archive hash, selected
transpiler seed/ISA circuit freeze and human approval.

The final verification pass for this audit had 49 tests passing, clean ruff and
clean mypy results.
